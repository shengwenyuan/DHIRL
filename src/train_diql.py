...
class DeepInverseQLearner:
    def __init__(self, reward_net, q_net, qsh_net, policy_net, 
                 lr=1e-3, gamma=0.99, tau=0.005, n_actions=None, device='cpu'):
    ...
    def train_on_batch(self, states, actions, next_states):
        ...
        with torch.no_grad():
            next_q_vals = self.target_q_net(next_states)  # [B, A]
            y_qsh = self.gamma * next_q_vals.max(dim=1, keepdim=True).values  # [B, A=1]
        ...
        # 4. Update Q
        with torch.no_grad():
            r_target = self.target_reward_net(states).gather(1, actions.unsqueeze(1))  # [B, 1]
            next_q = self.target_q_net(next_states).max(dim=1, keepdim=True).values   # [B, 1]
            y_q = r_target + self.gamma * next_q
    ...
    def train(self, states, actions, next_states, batch_size=64, epochs=10):
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, num_samples, batch_size):
                ...
                losses = self.train_on_batch(batch_states, batch_actions, batch_next_states)
...
if __name__ == "__main__":
    ...
    reward_net = R(state_dim, action_dim)
    q_net = Q(state_dim, action_dim)
    qsh_net = Qsh(state_dim, action_dim)
    ...
    for num_trajs in np.arange(1024, 1025, 100):
        for kf_idx, (train_idxes, test_idxes) in enumerate(kf.split(trajs[:num_trajs])):
            ...
            learner.train(D_states, D_actions, D_next_states, batch_size=32, epochs=10)
